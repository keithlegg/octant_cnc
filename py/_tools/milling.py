...
##----------------------------------------##
def resize_export(path, filename):
    vflo = vectorflow() 
    vflo.load_geojson('%s/%s.json'%(path, filename) ) 
    ...
    vflo.export_extents_ngc(path, 'nixe')
    vflo.export_ngc(.1, .1, .1, 2, '%s/nixe.ngc'%(path), do_laser=True, do3d=True, do_retracts=True, laserpwm=600)
